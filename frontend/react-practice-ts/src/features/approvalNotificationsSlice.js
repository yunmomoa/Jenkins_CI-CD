...
const approvalNotificationsSlice = createSlice({
    ...
    reducers: {
        clearNotification: (state, action) => {
            if (action.payload === "approvalFinish" || action.payload === "approvalReject")
                return;
    ...
    extraReducers: (builder) => {
        builder
            ...
            notifications.forEach((noti) => {
                if (noti.approvalLineType === "승인" && noti.status == 1)
                    state.approvalRequest++;
                else if (noti.approvalLineType === "수신")
                    state.approvalSend++;
                else if (noti.type === "참조자")
            ...
            approvalStatusData.forEach((noti) => {
                if (noti.approvalStatus === 2)
                    state.approvalFinish++;
                if (noti.approvalStatus === 3)
                    state.approvalReject++;
            });
        });
    },
});
