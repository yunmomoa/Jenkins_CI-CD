...
import styles from "./Calendar2.module.css"; // ✅ module.css 객체로 불러오기
...
function Calendar2({ onEventClick = () => {} }: Calendar2Props) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("addEventToCalendar2", handleAddEvent as EventListener);
    window.addEventListener("deleteEvent", handleDeleteEvent as EventListener);

    return () => {
      window.removeEventListener("addEventToCalendar2", handleAddEvent as EventListener);
